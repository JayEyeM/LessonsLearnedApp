...
const LessonsBoard = () => {
  ...
  if (loading) {
    return (
      <Box textAlign="center" py={20} color="whiteAlpha.800" fontSize="xl">
        <Spinner size="xl" thickness="4px" speed="0.65s" color="teal.300" mb={4} />
        Loading lessons...
      </Box>
    )
  }
  ...
  return (
    ...
    <Button
        ...
        variant="outline"
        mb={4}
        _hover={{ bg: "#1d0238ff", color: "orange.200", boxShadow: "0 0 10px rgba(227, 151, 255, 1)" }}
          ...
          return (
            <Box
              ...
              position="relative"
                ...
                boxShadow={isExpanded ? "0 0 10px 50px rgba(0, 0, 0, 1)" : "0 0 10px rgba(0, 0, 0, 1)"}
            >
              {isExpanded && (
                <IconButton
                  icon={<CloseIcon />}
              ...
              <Box display="flex" flexDirection="row" 
               gap={8} mb={2}>
                <Text fontSize="md" color="#ffffffff" mb={2}>Nickname:</Text>
              ...
              <Box display="flex" flexDirection="row" gap={8} mb={2}>
                <Text fontSize="0.75rem" color="#ffffffff" mb={2}>Date Posted:</Text>
                <Text fontSize="0.75rem" color="gray" mb={2}>
                  {new Date(lesson.post_created_date).toLocaleDateString()}
                </Text>
              </Box>

              <Box overflowY={isExpanded ? "visible" : "auto"} height={isExpanded ? "auto" : "180px"}>
                <Text fontWeight="bold" color="#4eb6cbff">Event:</Text>
                <Text mb={2}>{lesson.event_experienced}</Text>

                <Text fontWeight="bold" color="#4eb6cbff">Lesson:</Text>
                <Text mb={2}>{lesson.describe_lesson_learned}</Text>

                <Text fontWeight="bold" color="#4eb6cbff">Message:</Text>
